...
class InlineManager(
    ...
    :type client: astralix.tl_cache.CustomTelegramClient
    :type db: astralix.database.Database
    :type allmodules: astralix.loader.Modules
    """
    ...
    def __init__(
        ...
        self._token = db.get("astralix.inline", "bot_token", False)
    ...
    def _cleanup_stale_bot_sessions(self, bot_uid: str):
        prefix = f"astralix-{self._me}-bot-"
    ...
    async def register_manager(
        ...
        self._name = get_display_name(self._client.astralix_me)
        ...
        self._bot_client = TelegramClient(
            SQLiteSession(
                os.path.join(main.SESSIONS_DIR, f"astralix-{self._me}-bot-{bot_uid}")
            ),
            self._client.api_id,
        ...
        try:
            ...
            self._bot_client.astralix_me = bot_me
        ...
        for folder in _folders.filters:
            if getattr(folder, "title", None) == "astralix":
                if any(
                    [
    ...
    async def _ping_bot(
        ...
        try:
            m = await self._client.send_message(self.bot_username, "/start astralix init")
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start astralix init"
                )
            except Exception:
