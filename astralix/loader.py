    ...
    try:
        match name:
            ...
            case s if s.startswith("hikka"):
                return native_import("astralix" + name[5:], *args, **kwargs)
...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _astralix_client_id_logging_tag = copy.copy(  # noqa: F841
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?astralix$", from_id=659800858)
...
class Modules:
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.astralix_commands)
                inline_handlers.update(module.astralix_inline_handlers)
                callback_handlers.update(module.astralix_callback_handlers)
                watchers.extend(module.astralix_watchers.values())
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.astralix_commands))
            )

        for _command, cmd in instance.astralix_commands.items():
    ...
    def register_inline_stuff(self, instance: Module):
        ...
        for name, func in instance.astralix_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.astralix_watchers.values():
