...
@loader.tds
class astralixWebMod(loader.Module):
    """Inline account management"""

    strings = {"name": "astralixAccounts"}
    ...
    def _get_client(self) -> CustomTelegramClient:
        return CustomTelegramClient(
            MemorySession(),
            main.astralix.api_token.ID,
            main.astralix.api_token.HASH,
            connection=main.astralix.conn,
            proxy=main.astralix.proxy,
    ...
    def _clone_switch_db(self, old_id: int, new_id: int) -> dict:
        ...
        inline_data = data.setdefault("astralix.inline", {})
    ...
    async def _save_switch_session(
        ...
        session = SQLiteSession(os.path.join(main.SESSIONS_DIR, f"astralix-{new_id}"))
        session.set_dc(
    ...
    async def _switch_account(self, client: CustomTelegramClient) -> None:
        ...
        new_id = me.id
        if old_id == new_id:
            await main.astralix.save_client_session(client, delay_restart=False)
            return

        suffix = int(time.time())
        base = Path(main.SESSIONS_DIR)
        old_session = base / f"astralix-{old_id}.session"
        old_journal = base / f"astralix-{old_id}.session-journal"
        old_config = Path(main.BASE_PATH) / f"config-{old_id}.json"
        new_session = base / f"astralix-{new_id}.session"
        new_journal = base / f"astralix-{new_id}.session-journal"
