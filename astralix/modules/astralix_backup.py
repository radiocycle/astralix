...
@loader.tds
class astralixBackupMod(loader.Module):
    """Handles database and modules backups"""

    strings = {"name": "astralixBackup"}

    async def client_ready(self):
        if not self.get("period"):
            await self.inline.bot.send_photo(
                self.tg_id,
                photo="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/astralix/unit_alpha.png",
    ...
    async def handler(self):
        try:
                ...
                reply_markup=self.inline.generate_markup(
                    [
                        [
                            {
                                "text": "↪️ Restore this",
                                "data": "astralix/backupall/restore/confirm",
                            }
                        ]
                    ]
                ),
            ...
            logger.exception("astralixBackup failed")
            await asyncio.sleep(60)

    @loader.callback_handler()
    async def restore(self, call: BotInlineCall):
        if not call.data.startswith("astralix/backupall/restore"):
            return

        if call.data == "astralix/backupall/restore/confirm":
            await utils.answer(
                call,
                "❓ <b>Are you sure?</b>",
                reply_markup={
                    "text": "✅ Yes",
                    "data": "astralix/backupall/restore",
                },
            )
            return
        ...
        try:
            ...
            with zipfile.ZipFile(zipfile_bytes) as zf:
                ...
                with contextlib.suppress(KeyError):
                    db_data["astralix.inline"].pop("bot_token")
    ...
    def _convert(self, backup):
        fixed = re.sub(r"(hikka\.)(\S+\":)", lambda m: "astralix." + m.group(2), backup)
    ...
    async def restoredb(self, message: Message):
        ...
        with contextlib.suppress(KeyError):
            decoded_text["astralix.inline"].pop("bot_token")
    ...
    async def backupall(self, message: Message):
        ...
        archive.name = f"astralix-{datetime.datetime.now():%d-%m-%Y-%H-%M}.backup"
