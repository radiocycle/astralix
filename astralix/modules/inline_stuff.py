...
class InlineStuff(loader.Module):
    ...
    @loader.command()
    async def ch_astralix_bot(self, message: Message):
        ...
        self._db.set("astralix.inline", "custom_bot", args)
    ...
    async def ch_bot_token(self, message: Message):
        ...
        if not args or not re.match(r"[0-9]{8,10}:[a-zA-Z0-9_-]{34,36}", args):
            ...
            return
        self._db.set("astralix.inline", "bot_token", args)
        await utils.answer(message, self.strings["bot_updated"])
        ...
        match message.text:
            case "/start":
                await message.answer_photo(
                    "https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/astralix/start_cmd.png",
    ...
    async def reset_prefix(self, call: InlineCall, message):
        ...
        self.db.set("astralix.main", "command_prefix", ".")
