...
class LoaderMod(loader.Module):
    ...
    async def approve_internal(
        ...
        await call.edit(
            ...
            photo="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/astralix/joined_jr.png",
        )
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?astralix_min", doc):
            ver = re.search(r"# ?scope: ?astralix_min ((?:\d+\.){2}\d+)", doc).group(1)
            ver_ = tuple(map(int, ver.split(".")))
            if main.__version__ < ver_:
                logger.error(
                    "Module %s requires astralix %s, current version is %s",
                    module_label,
                    ver,
        ...
        module_name = f"astralix.modules.{uid}"
        ...
        try:
            try:
                ...
                async def inner_proxy():
                    ...
                    while True:
                        if hasattr(instance, "astralix_wait_channel_approve"):
                            if message:
                                (
                                    module,
                                    channel,
                                    reason,
                                ) = instance.astralix_wait_channel_approve
                                message = await utils.answer(
                                    message,
        ...
        instance.astralix_meta_pic = next(
            (
        ...
        try:
            developer_entity = await (
                ...
                if (
                    developer in self._client.astralix_entity_cache
                    and getattr(
    ...
    def _is_core_module(self, module) -> bool:
        ...
        if not module_name.startswith("astralix.modules."):
            return False
