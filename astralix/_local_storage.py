...
class LocalStorage:
    ...
    def __init__(self):
        self._path = os.path.join(os.path.expanduser("~"), ".astralix", "modules_cache")
    ...
    async def fetch(self, url: str, auth: str | None = None) -> str:
        ...
        try:
            r = await utils.run_sync(
                ...
                headers={
                    "User-Agent": "astralix Userbot",
                    "X-astralix-Version": ".".join(map(str, __version__)),
                    "X-astralix-Commit-SHA": utils.get_git_hash(),
                    "X-astralix-User": str(self._client.tg_id),
                },
            )
            r.raise_for_status()
        except Exception:
