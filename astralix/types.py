...
class Module:
    ...
    @property
    def astralix_commands(self) -> dict[str, Command]:
        """List of commands that module supports"""
        return get_commands(self)
    ...
    @property
    def astralix_inline_handlers(self) -> dict[str, Command]:
        """List of inline handlers that module supports"""
        return get_inline_handlers(self)
    ...
    @property
    def astralix_callback_handlers(self) -> dict[str, Command]:
        """List of callback handlers that module supports"""
        return get_callback_handlers(self)
    ...
    @property
    def astralix_watchers(self) -> dict[str, Command]:
        """List of watchers that module supports"""
        return get_watchers(self)
    ...
    @astralix_commands.setter
    def astralix_commands(self, _):
        pass
    ...
    @astralix_inline_handlers.setter
    def astralix_inline_handlers(self, _):
        pass
    ...
    @astralix_callback_handlers.setter
    def astralix_callback_handlers(self, _):
        pass
    ...
    @astralix_watchers.setter
    def astralix_watchers(self, _):
        pass
    ...
    async def animate(
        ...
        *,
        ...
        with contextlib.suppress(AttributeError):
            _astralix_client_id_logging_tag = copy.copy(self.client.tg_id)  # noqa: F841
    ...
    async def _decline(
        ...
        self._db.set(
            "astralix.main",
            "declined_joins",
            list(set(self._db.get("astralix.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
            ...
            photo="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/astralix/declined_jr.png",
        )
    ...
    async def request_join(
        ...
        match channel:
            ...
            case _ if channel.id in self._db.get("astralix.main", "declined_joins", []):
                if assure_joined:
                    raise LoadError(
        ...
        await self.inline.bot.send_photo(
            self.tg_id,
            "https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/astralix/join_request.png",
            caption=(
        ...
        self.astralix_wait_channel_approve = (
            self.__class__.__name__,
            channel,
            reason,
        )
        ...
        with contextlib.suppress(AttributeError):
            delattr(self, "astralix_wait_channel_approve")
    ...
    async def import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?astralix_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?astralix_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires astralix version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"astralix.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("astralix.main", "stats", True)
            and url is not None
