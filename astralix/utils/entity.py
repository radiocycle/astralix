...
async def asset_forum_topic(
    ...
    async def create_topic() -> ForumTopic:
        result = await client(
            CreateForumTopicRequest(
                peer=entity,
                title=title,
                icon_emoji_id=(icon_emoji_id if client.astralix_me.premium else None),
            )
        )
    ...
    forums_cache = db.pointer("astralix.forums", "forums_cache", {})
    ...
    async def _search_topic(topic_title: str) -> int | None:
        ...
        for found_topic in result.topics:
            if found_topic.title == topic_title:
                forums_cache.setdefault("astralix-userbot", {})[topic_title] = found_topic.id
                return found_topic.id
        return None

    if topic_id := forums_cache.get("astralix-userbot", {}).get(title) or await _search_topic(
        title
    ):
        await fw_protect()
        ...
        if isinstance(new_topic, ForumTopicDeleted):
            ...
            forums_cache["astralix-userbot"][title] = new_topic.id

    else:
        await fw_protect()
        new_topic = await create_topic()
        forums_cache.setdefault("astralix-userbot", {})[title] = new_topic.id
...
async def wait_for_content_channel(db: "Database", delay: float = 10) -> int:
    cid = db.get("astralix.forums", "channel_id", None)

    while not cid:
        logger.warning(
            "astralix content channel not found in database. Sleeping 10 seconds..."
        )
...
async def get_topic_id(db: "Database", topic_name: str) -> int | None:
    ...
    """
    try:
        forums_cache = db.get("astralix.forums", "forums_cache", {})
        return forums_cache.get("astralix-userbot", {}).get(topic_name)
    except Exception:
        return None
