...
class astralix:
    ...
    def _migrate_sessions(self):
        ...
        with os.scandir(BASE_DIR) as entries:
            legacy = [
                ...
                if entry.is_file()
                and entry.name.startswith("astralix-")
    ...
    async def save_client_session(
        ...
        *,
        ...
        if hasattr(client, "tg_id"):
            ...
            client.hikka_me = me
            client.astralix_me = me

        session = SQLiteSession(
            os.path.join(
                SESSIONS_DIR,
                f"astralix-{telegram_id}",
            )
        )
        ...
        client.astralix_db = database.Database(client)
        await client.astralix_db.init()

        try:
            db = client.astralix_db
            existing = db.get("astralix.inline", "custom_bot", False)
        except Exception:
            existing = False
        ...
        if (
            ...
            while bot := input(
                "You can enter a custom bot username or leave it empty and astralix will generate a random one: "
            ):
                bot = bot.strip()
                bot = bot.lstrip("@")
                if any(
                ...
                try:
                    if await self._check_bot(client, bot):
                        db.set("astralix.inline", "custom_bot", bot)
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            if os.environ.get("astralix_NO_GIT") == "1":
                build = "unknown"
                upd = "Git disabled"
            else:
                import git
            ...
            pref = client.astralix_db.get("astralix.main", "command_prefix", None)
            ...
            logging.debug(
                ...
                client.tg_id,
                client.astralix_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
