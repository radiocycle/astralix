...
def getlines(filename: str, module_globals=None) -> str:
    ...
    source code of astralix modules properly.
    """

    try:
        if filename.startswith("<") and filename.endswith(">"):
            module = filename[1:-1].split(maxsplit=1)[-1]
            if (module.startswith("astralix.modules")) and module in sys.modules:
                return list(
                    map(
                        lambda x: f"{x}\n",
...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.astralix_caller or client_id == record.astralix_caller)
        ]
    ...
    async def sender(self):
        async with self._send_lock:
            ...
            for client_id in self._mods:
                ...
                funcs = []
                for item in self.tg_buff:
                    if not isinstance(item[0], astralixException):
                        continue
            ...
            for client_id in self._mods:
                ...
                if len(self._queue[client_id]) > 5:
                    ...
                    logfile.name = "astralix-logs.txt"
                    logfile.seek(0)
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_astralix_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_astralix_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        if record.levelno >= self.tg_level:
            if record.exc_info:
                ...
                exc = astralixException.from_exc_info(
                    *record.exc_info,
...
def init():
    ...
    logging.getLogger("telethon.network").addFilter(NoFetchUpdatesFilter())
