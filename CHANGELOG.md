...
## 🪐 astralix 2.1.0
 ...
 - add -w and --wipe argument for python3 -m astralix for wipe session, databases, loaded modules
 ...
 - migrate inline bot from aiogram to astralixTL
 ...
 - fix(astralix_config): simplify formatting of option lines
...
- remove "astralix" from bot username
- fix asset channel
...
- remove invokecmd
- remove uninstall_astralixcmd
- remove debugmodcmd
- add new validator RandomLink
- replace validators in help, astralixinfo, tester to RandomLink
...
- remove show_astralix from info
...
- loging improvements
- add invert_media in astralix-tl
- fix zero CPU %
- added config split
...
- Telethon has been completely updated, moving to astralix-tl
- update astralix-tl to 1.1.0
- added prefix to ping
- removed official support serv00, tothost, goorm, railway, termux
- rename folder "hikka" to "astralix"
- fix close btn
- fix native imports
- added converting Hikka backups to astralix
...
- added config for show "astralix" in info
- removed UnitHeta
...
- Drop Okteto support
- Show astralix platform and version in Telegram sessions list
...
- Completely drop astralix support due to legacy code, limits and removing of free tier
...
- Publish hikka telethon fork and migrate to it in requirements, thereby fixing the deployment error on astralix
- Add custom emojis filter to `utils.remove_html`
- Fix `client.get_perms_cached`
- Fix translation flaw in `astralixSecurity`
...
- Add platform-specific astralix emojis to premium users
...
- Add support for multiaccounting on astralix
...
- Remove `astralix3` from classic requirements, along with astralix installation code snippet
...
- Update quickstart
- New fields in astralixInfo
- Remove suffix `astralix` if user specified it somewhere below
...
- Fix astralix-specific config error
...
- Add automatic webpage bot unblock in astralix waker
- Add secure boot feature
- Update native astralix postgre database saving method
...
- Add `banner_url` config var to `astralixInfo`
- Add `loader.validators.Hidden`
- Add `websockets` dependency, so users can load hikarichat on astralix
...
- Add `client.force_get_entity` to bypass astralix Cacher
...
- Fix translation issue in astralixConfig
- Fix `.dump`
...
- Add additional astralix deps
- Fix `load_module` reattempt
- Reorder database read-writes to make postgres the ladder
- Make redis optional for non-astralix users
- Add source to Pipfile
- New astralix dependencies list in Pipfile
...
- Add banner with astralix installation status
- Reorder config saving in web
- Fix `Unauthorized` error on astralix
...
- Update gitignore so git doesn't count shit on astralix
- Visual astralix fixes in updater
- Deepsource fixes
- Add `utils.get_entity_url`, `utils.get_message_link`, `utils.remove_html`, `utils.get_kwargs`
- Disable modules debugging on astralix
...
- Refactor `astralix.py` app searching algorithm
- Refactor postgresql database saving process
- Fix astralix restart message not being edited
- Add astralix waker
- Make `hikka-` app naming optional

## 🌑 astralix 1.2.0

- Add full-featured astralix support with additional buildpacks
...
- Drop Uniborg and Raphielgang compatibility layer. More info in [2d525ab](https://github.com/hikariatama/astralix/commit/2d525ab6b0e6b9d7ccd7408bbe175cea24d780a5)
- Minor bug fixes

## 🌑 astralix 1.1.0

- Make inline commands' docstrings translatable
- Introducing astralixDynamicTranslate - the possibility for developers to translate their own modules to other language without usage of native translates
...
- Refactor astralixSecurity, especially the bounding mask control
...
- Add ability to disable astralixDL natively
- Fix Updater so it works if you have troubles with inline mode

## 🌑 astralix 1.0.22

Add astralixDL module to use inline download buttons in verified channels
